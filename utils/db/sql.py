...
async def create_user_db():
    create_db_command = open(os.path.join(sys.path[0], "utils/db/user_db.sql"), "r").read()
...
async def create_blog_db():
    create_db_command = open(os.path.join(sys.path[0], "utils/db/blog_db.sql"), "r").read()

    logging.info("Connecting to blog table...")
    conn: asyncpg.Connection = await asyncpg.connect(user=PG_USER, password=PG_PASS, host=HOST)
    ...
    try:
        ...
        pass
    await conn.close()
    logging.info("Table BLOG created")
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(create_blog_db())
